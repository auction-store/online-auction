...
class AuctionDetail extends React.Component {
  ...
  handleNewBid() {
    if (this.state.bid === "") {
      this.showFade("First enter your offer.");
      return;
    }
    if (this.state.bid < this.state.sale.firstPrice) {
      const message = (
        <span>
          You cannot bid lower than the starting price. (
          <Currency
    ...
    if (this.state.bid < this.state.maxBid.amount) {
      this.showFade(
        "En yüksek tekliften düşük teklif veremezsiniz. (" +
          this.state.maxBid.amount +
          "₺)"
      );
      return;
    }
    if (this.state.bid == this.state.maxBid.amount) {
      this.showFade(
        this.state.bid + "₺ en yüksek teklif, aynı teklifi vermezsiniz."
      );
      return;
    }
  ...
  getSale(id) {
    ...
    axios
      ...
      .then((response) => {
        var maxBid =
          response.data.sale.bids.length === 0
            ? {
                bidder: {
                  email: "",
                  nameSurname: "",
      ...
      <Layout page="home" user={this.state.user} loggedIn={this.state.loggedIn}>
        ...
        >
          <h2>
            ...
            {userEmail === this.state.sale.owner.email ? (
          ...
          <div className="row">
            ...
            <div className="col-md-4 pt-4">
              <Bidding
                ...
                maxBidder={
                  loggedIn &&
                  this.state.maxBid.bidder.email === this.state.user.email
                }
                bid={this.state.bid}
          ...
          <div className="row mt-3">
            <div className="col-md-4">
              <p className="mb-4 mt-4">
                <FontAwesomeIcon icon={faUser} width="16" /> <b>Seller: </b>
                {this.state.sale.owner.nameSurname}
              </p>
              <p className="mb-4">
                <FontAwesomeIcon icon={faMoneyBill} width="16" />{" "}
                <b>Starting Price: </b>
                <Currency
                  quantity={this.state.sale.firstPrice}
                  currency="TRY"
            ...
            <div className="col-md-4">
              ...
              <p className="mb-4">
                <FontAwesomeIcon icon={faThumbsUp} width="16" /> <b>Status: </b>
                {this.state.sale.state}
              </p>
            </div>
            <div className="col-md-4">
              <p className="mb-4 mt-4">
                <FontAwesomeIcon icon={faCalendarAlt} width="16" />{" "}
                <b>Closing: </b>
                {endDate}
              </p>
              <p>
                <FontAwesomeIcon icon={faMapMarkerAlt} width="16" />{" "}
                <b>Location: </b>
