...
class CountdownNumber extends React.Component {
  render() {
    return (
      <span
        style={{ fontSize: "20px", fontFamily: "Verdana", display: "block" }}
      >
        {this.props.number}
      </span>
    );
  ...
  render() {
    return (
      <div className="row pr-2 pl-2">
        <CountdownBox number={this.state.day} title="DAY" />
        <CountdownBox number={this.state.hour} title="HOUR" />
        <CountdownBox number={this.state.minute} title="MINUTE" />
        <CountdownBox number={this.state.second} title="SECOND" />
      </div>
    );
  }
}
